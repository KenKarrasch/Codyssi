...
def shortest_path_in_grid(grid, start, end):
    """
    Find the shortest path cost from start to end in a grid where movement cost
    is the value of the destination cell (U/D/L/R moves only).
    
    Args:
        grid: 2D list of numbers (assumed to be rectangular)
        start: Tuple (row, col) of starting position (1-based)
        end: Tuple (row, col) of target position (1-based)
    
    Returns:
        Tuple of (total_cost, path) where path is a list of coordinates
    """
...
total_cost, path = shortest_path_in_grid(grid, (1, 1), (15, 15))
